...
export default function GardenMeisterFullApp() {
  ...
  if (view === "rules") {
    return (
      <div className="min-h-screen bg-stone-100 p-8 flex flex-col items-center animate-in slide-in-from-right duration-500">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-5xl mb-16">
          {[
            { n: "01", t: "植える", d: "1ターン3アクション。4色のチューリップを盤面に配置します。各色10個ずつの在庫を管理しましょう。" },
            { n: "02", t: "揃える", d: "右側の「設計図」と同じ配置を作ると得点。回転・反転・逆順のどれでも達成とみなされます。" },
            { n: "03", t: "替える", d: "配置が難しい時は、1アクションを消費してカードをクリックし、新しい設計図を引き直せます。" }
          ].map((item, i) => (
            <div key={i} className="bg-white p-8 rounded-3xl shadow-md border-b-4 border-stone-200">
        ...
        <div className="flex gap-4">
          <button onClick={() => setView("home")} className="bg-stone-300 text-stone-700 px-8 py-4 rounded-2xl font-black hover:bg-stone-400">BACK</button>
  ...
  return (
    <div className="animate-in fade-in duration-1000">
      <button onClick={() => { if(confirm("終了しますか？")) setView("home"); }} className="fixed top-4 left-4 z-[60] bg-white/80 backdrop-blur px-4 py-2 rounded-full text-[10px] font-black shadow-sm border border-stone-200 hover:bg-white">← EXIT TO MENU</button>
      ...
      <main className="flex min-h-screen flex-col md:flex-row items-center md:items-start justify-center p-4 md:p-8 font-sans gap-8 bg-stone-100 text-stone-800">
        {/* 左側: 3D盤面 & リザルト */}
        <div className={`relative w-full max-w-[500px] aspect-square rounded-3xl shadow-2xl border-[8px] md:border-[16px] transition-all duration-500 overflow-hidden bg-white ${isGameOver ? 'border-stone-400' : currentPlayer === 1 ? 'border-[#8b4513]' : 'border-[#556b2f]'}`}>
          ...
          {isGameOver && (
            <div className="absolute inset-0 bg-stone-900/90 backdrop-blur-md flex flex-col items-center justify-center text-white p-8 z-50 animate-in zoom-in">
              <p className="text-amber-500 font-black text-sm mb-2 tracking-widest uppercase italic">{isBoardFull ? "Garden Fully Completed!" : "Designs Exhausted!"}</p>
              <h2 className="text-5xl font-black mb-10 italic">{winner === 0 ? "EXCELLENT DRAW" : `P${winner} VICTORIOUS!`}</h2>
              ...
              <button onClick={() => { resetGame(); setView("game"); }} className="bg-white text-stone-900 px-10 py-4 rounded-2xl font-black shadow-2xl hover:bg-amber-500 hover:text-white transition-all">REPLAY</button>
            </div>
          )}
        </div>
        ...
        <div className="md:ml-8 w-80 space-y-4">
          <div className="bg-white p-6 rounded-3xl shadow-xl border-b-8 border-stone-300">
            ...
            <div className="flex gap-2 mb-6 text-center font-black">
              {[1, 2].map(p => (
                <div key={p} className={`flex-1 p-2 rounded-xl border-2 transition-all ${currentPlayer === p ? 'border-amber-600 bg-amber-50' : 'border-stone-50 opacity-40'}`}>
                  <p className="text-[10px] uppercase">P{p}</p><p className="text-2xl">{scores[p as 1|2]}</p>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3 mb-6">
              <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest px-1">Active Plans ({deck.length})</p>
              {faceUpCards.map((card, idx) => (
                <div key={card.id} onClick={() => {
                  ...
                  <div className="absolute inset-0 bg-amber-600/90 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity text-white text-[10px] font-black">EXCHANGE (-1 ACT)</div>
            ...
            <div className="grid grid-cols-4 gap-2 mb-6">
              {PIECE_TYPES.map(p => (
                <button key={p.id} onClick={() => setSelectedColor(p.color)} className={`p-2 rounded-xl border-2 transition-all ${selectedColor === p.color ? 'border-amber-500 bg-amber-50 shadow-sm' : 'border-transparent bg-stone-50'}`}>
                  <div className={`w-5 h-5 mx-auto ${p.color} rounded-full border border-black/5`} />
                  <span className="text-[10px] font-bold text-stone-500 mt-1 block">{INITIAL_STOCK - stocks[p.color]}</span>
                </button>
              ))}
            </div>
            ...
            <div className="mb-4 text-center">
              <div className="flex justify-center gap-1 mb-2">
                {[...Array(MAX_ACTIONS)].map((_, i) => (
                  <div key={i} className={`h-1.5 w-6 rounded-full ${i < actions ? 'bg-green-500' : 'bg-stone-200'}`} />
                ))}
              </div>
            </div>
